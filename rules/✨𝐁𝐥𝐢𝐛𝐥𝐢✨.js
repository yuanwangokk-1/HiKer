const csdown = {
  ...
  home: () => {
    ...
    try {
        ...
        storage0.getItem("classify_").forEach((data) => {
          d.push({
            title:
              ...
              data.classifyId + ""
                ? strong(data.classifyTitle, "FF6699")
                : data.classifyTitle,
      ...
      if (
        getMyVar("classify", getMyVar("index_classify")) == 1 ||
        getMyVar("classify", getMyVar("index_classify")) == 8
      ) {
        let url =
          getItem("host") +
          "/api/station/stations?classifyId=" +
          getMyVar("classify", getMyVar("index_classify")) +
          "&page=" +
          pg +
          "&pageSize=10";
        ...
        getMyVar("classify", getMyVar("index_classify")) == 2 ||
        getMyVar("classify", getMyVar("index_classify")) == 9 ||
        getMyVar("classify", getMyVar("index_classify")) == 6
      ) {
        if (MY_PAGE == 1) {
          let 动漫分类 = [
            {
          ...
          Cate(
            动漫分类,
            "动漫分类" + getMyVar("classify", getMyVar("index_classify")),
            d
          );
          ...
          d.push({
            title:
              getItem(
                "year" + getMyVar("classify", getMyVar("index_classify")),
                storage0.getItem("weeklist")[0].weekRanges[0].year + ""
              ) +
              "年 第" +
              getItem(
                "week" + getMyVar("classify", getMyVar("index_classify")),
                storage0.getItem("weeklist")[0].weekRanges[0].week + ""
              ) +
              "周 " +
              getItem(
                "startDate" + getMyVar("classify", getMyVar("index_classify")),
                storage0.getItem("weeklist")[0].weekRanges[0].startTime
              ) +
              "~" +
              getItem(
                "endDate" + getMyVar("classify", getMyVar("index_classify")),
                storage0.getItem("weeklist")[0].weekRanges[0].endTime
              ),
            url: $("#noLoading#").lazyRule(() => {
              let option = [];
        ...
        let url =
          getItem("host") +
          "/api/video/getByClassify?pageSize=10&page=" +
          pg +
          "&sortType=" +
          getMyVar(
            "动漫分类" + getMyVar("classify", getMyVar("index_classify")),
        ...
        getMyVar("classify", getMyVar("index_classify")) == 3 ||
        getMyVar("classify", getMyVar("index_classify")) == 7
      ) {
        if (MY_PAGE == 1) {
          let 漫画分类 = [
            {
        ...
        let token_data = JSON.parse(
          fetch(token_url, {
            headers: {
              ...
              "User-Agent":
                "Mozilla/5.0 (Linux; Android 15; 24031PN0DC Build/AQ3A.240627.003; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/130.0.6723.86 Mobile Safari/537.36;SuiRui/acfan/ver=1.0.2",
            },
            body: {
              deviceId: getItem("deviceId"),
              acb: "Bp",
    ...
    var pic = $("").lazyRule(() => {
      ...
      try {
        ...
        for (item of data.imgList) {
          img.push(
            "http://caoliusp.xka2a.top/z_tupiandaili/caoliusp.php?url=" +
              getItem("imgDomain") +
              item +
              getItem("ppp", "")
          );
  ...
  search: () => {
    ...
    let url =
      getItem("host") +
      "/api/search/keyWord?pageSize=15&page=" +
      pg +
      "&searchWord=" +
      getMyVar("keyword") +
      "&searchType=" +
  ...
  videoerji: () => {
    ...
    var url =
      getItem("host") +
      "/api/station/getStationMore?stationId=" +
      id +
      "&page=" +
      pg +
      "&pageSize=12";
