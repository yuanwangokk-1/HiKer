const csdown = {
  ...
  aes: $.toString(() => {
    ...
    function post(url, body) {
      ...
      let sign = md5(
        "body" + data + "appIdsc04requestId" + requestId + "time" + t
      );
      let body =
        '{"requestId":"' +
        requestId +
    ...
    var vod = $("").lazyRule(() => {
      ...
      let url1 =
        getItem("Domain") +
        post(vod_url, vod_body).data.videoInfo.masterVideoUrl;
    ...
    try {
      if (getMyVar("搜索", "1") == 2) {
        ...
        data.forEach((data) => {
          d.push({
            ...
            desc:
              data.publishTime +
              "  " +
              parseInt(data.videoInfo.videoDuration / 60) +
              ":" +
              parseInt(data.videoInfo.videoDuration % 60),
        ...
        let body =
          '{"pageNo":' +
          pg +
          ',"pageSize":10,"searchWord":"' +
          getMyVar("keyword") +
          '","tagName":null,"mediaType":21}';
        let data = post(url, body).data.dataList;
        data.forEach((data) => {
          d.push({
            title: data.title,
              ...
              if (data.type == "image") {
                ...
                d.push({
                  ...
                  url: $("noLoading#").lazyRule((content) => {
                    ...
                    let data =
                      getItem("Domain") +
                      JSON.parse(Decrypt(fetch(getItem("Domain") + content)))[0]
                        .mainUrl;
                    return data;
                  }, content),
                  col_type: "card_pic_1",
  ...
  black: () => {
    ...
    try {
      if (MY_PAGE == 1) {
        if (!getMyVar("host", "")) {
          let host_arr = [
            "https://www.cqwp4szs.cc",
            "https://www.5hdk34pf15.vip",
          ];
          for (let item of host_arr) {
            let data = JSON.parse(
              fetch(item + "/front/test", {
                withStatusCode: true,
                timeout: 5000,
              })
            );
        ...
        if (storage0.getMyVar("cdnline") == "") {
          let cdnline_url =
            getMyVar("host") + "/front/system/cdnline/userGetCdn";
        ...
        if (!storage0.getItem("cate_")) {
          let cate_url =
            getMyVar("host") + "/front/long/category/userGetCategory";
        ...
        storage0.getItem("cate_")[0].childList.forEach((data) => {
          d.push({
            title:
              getMyVar("black", getMyVar("index_black")) == data.categoryId + ""
                ? strong(data.categoryName, "FF6699")
                : data.categoryName,
            url: $("#noLoading#").lazyRule(
              (title, id) => {
                putMyVar("black", id);
      ...
      if (
        getMyVar("black", getMyVar("index_black")) == getMyVar("index_black")
      ) {
        var black_body =
          '{"pageNo":' +
          pg +
          ',"pageSize":10,"mediaType":21,"publishDate":null,"payType":null,"topicId":"","subjectId":null,"publishStatus":1,"recommend":null,"firstCategoryId":null,"secondCategoryId":"","thirdCategoryId":"","sort":"' +
          getMyVar("黑料分类", "publish_time") +
          '"}';
      } else if (
        getMyVar("black", getMyVar("index_black")) == "1821892692577890304"
      ) {
        var black_body =
          '{"pageNo":' +
          pg +
          ',"pageSize":10,"mediaType":21,"publishDate":null,"payType":null,"topicId":null,"subjectId":null,"publishStatus":1,"recommend":1,"firstCategoryId":null,"secondCategoryId":null,"thirdCategoryId":null,"sort":"' +
          getMyVar("黑料分类", "publish_time") +
          '"}';
      } else {
        var black_body =
          '{"pageNo":' +
          pg +
          ',"pageSize":10,"mediaType":21,"publishDate":null,"payType":null,"topicId":"","subjectId":null,"publishStatus":1,"recommend":null,"firstCategoryId":null,"secondCategoryId":"' +
          getMyVar("black", getMyVar("index_black")) +
          '","thirdCategoryId":"","sort":"' +
          getMyVar("黑料分类", "publish_time") +
          '"}';
      }
  ...
  blackerji: () => {
    ...
    try {
      ...
      let content = JSON.parse(
        Decrypt(fetch(getItem("Domain") + article_data.content))
      );
      d.push({
    ...
    try {
      if (MY_PAGE == 1) {
        ...
        storage0.getItem("cate_")[1].childList.forEach((data) => {
          d.push({
            title:
              getMyVar("video", getMyVar("index_video")) == data.categoryId + ""
                ? strong(data.categoryName, "FF6699")
                : data.categoryName,
            url: $("#noLoading#").lazyRule(
              (title, id) => {
      ...
      if (
        getMyVar("video", getMyVar("index_video")) == "1821892098983211008" ||
        getMyVar("video", getMyVar("index_video")) == "1836975781077463040"
      ) {
        if (MY_PAGE == 1) {
          let video_url =
            getMyVar("host") +
            "/front/long/handle/listAllGroupBySecondCategoryId";
          let video_body =
            '{"categoryId":"' +
            getMyVar("video", getMyVar("index_video")) +
            '"}';
        ...
        if (
          getMyVar("video", getMyVar("index_video")) == "1821892179224440832"
        ) {
          var video_url =
            getMyVar("host") + "/front/long/handle/userGetVideoBySubjectId";
          var video_body =
            '{"subjectId":null,"orderType":"' +
            getMyVar("片库分类", "publish_time") +
            '","recommend":null,"pageNo":' +
            pg +
            ',"pageSize":10}';
        } else {
          var video_url =
            getMyVar("host") + "/front/long/handle/userGetVideoByCategoryId";
          var video_body =
            '{"categoryId":"' +
            getMyVar("video", getMyVar("index_video")) +
            '","orderType":"' +
    ...
    try {
      if (MY_PAGE == 1) {
        ...
        storage0.getItem("cate_")[4].childList.forEach((data) => {
          d.push({
            title:
              getMyVar("topic", getMyVar("index_topic")) == data.categoryId + ""
                ? strong(data.categoryName, "FF6699")
                : data.categoryName,
            url: $("#noLoading#").lazyRule(
              (title, id) => {
        ...
        let topicpage_body =
          '{"pageNo":' +
          pg +
          ',"pageSize":6,"title":null,"categoryId":null,"topicId":null,"recommend":1,"firstCategoryId":null,"secondCategoryId":"' +
          getMyVar("topic", getMyVar("index_topic")) +
          '","thirdCategoryId":null,"sort":null}';
      ...
      let topic_body =
        '{"pageNo":' +
        pg +
        ',"pageSize":10,"mediaType":3,"publishDate":null,"payType":null,"topicId":null,"subjectId":null,"publishStatus":1,"recommend":null,"firstCategoryId":null,"secondCategoryId":"' +
        getMyVar("topic", getMyVar("index_topic")) +
        '","thirdCategoryId":null,"sort":"' +
        getMyVar("片库分类", "publish_time") +
        '"}';
  ...
  uperji: () => {
    ...
    try {
      ...
      let body =
        '{"pageNo":' +
        pg +
        ',"pageSize":10,"mediaType":3,"userId":"' +
        id +
        '","publishStatus":1,"sort":null}';
      let data = post(url, body).data.dataList;
      data.forEach((data) => {
        d.push(
          {
  ...
  qunerji: () => {
    ...
    try {
      ...
      let body =
        '{"pageNo":' +
        pg +
        ',"pageSize":10,"mediaType":3,"payType":null,"topicId":"' +
        id +
        '","publishStatus":1,"recommend":null,"sort":"' +
        getMyVar("圈子分类", "publish_time") +
        '"}';
      let data = post(url, body).data.dataList;
      data.forEach((data) => {
        d.push(
  ...
  token: () => {
    ...
    let t = Math.floor(Date.now());
    let data = Encrypt(
      '{"merchantAcct":"sc04","deviceId":"' + getItem("deviceId") + '"}'
    );
    let sign = md5(
      "body" + data + "appIdsc04requestId" + requestId + "time" + t
    );
