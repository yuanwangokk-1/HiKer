const mls = {
  ...
  home: () => {
    ...
    try {
      var 视频 = [
        {
          title:
            "推荐&最新&原创自制&免费&热门福利姬&诱惑小萝莉&网络热门专区&cosplay&乱伦偷情&伪娘百合同性&欧美少女&日韩",
      ...
      if ((getMyVar("视频", "1") == 32) | (getMyVar("视频", "1") == 43)) {
        ...
        var url =
          getItem("host") +
          "/api/video/classify/getClassifyVideos?pageSize=30&page=" +
          pg +
          "&classifyId=" +
          getMyVar("视频", "1") +
          "&sortNum=" +
        ...
        var url =
          getItem("host") +
          "/api/video/getStations?classifyId=" +
          getMyVar("视频", "1");
        let html2 = post(url);
        var list = JSON.parse(html2).data;
        var domain = JSON.parse(html2).domain;
        list.forEach((data) => {
              ...
              hotRank.forEach((data) => {
                d.push({
                  ...
                  desc:
                    ...
                    parseInt(data.playTime % 60),
                  url:
                    data.price == 0 ? data.videoId + vod1 : data.videoId + vod,
                  img:
                    "http://caoliusp.xka2a.top/z_tupiandaili/caoliusp.php?url=" +
                    domain +
                    data.coverImg[0],
          ...
          try {
            ...
            mainVideo.forEach((data) => {
              dpush({
                title:
                  (data.price == 0 ? data.title : "VIP  " + data.title) +
                  "   " +
                  data.createdAt.split(".")[0] +
                  "\t\t\t\t" +
                  parseInt(data.playTime / 60) +
        ...
        let token_data = JSON.parse(
          fetch(token_url, {
            headers: {
              ...
              deviceId: deviceId,
              "User-Agent":
                "Mozilla/5.0 (Linux; Android 14; 22081212C Build/UKQ1.230917.001; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/118.0.0.0 Mobile Safari/537.36;SuiRui/mh/ver=1.7.8",
            },
            body: {
              deviceId: deviceId,
              dew: "K",
  ...
  videoerji: () => {
    ...
    var url =
      getItem("host") +
      "/api/video/getStationMore?pageSize=20&page=" +
      pg +
      "&sortType=" +
      getMyVar("视频二级", "1") +
      "&stationId=" +
