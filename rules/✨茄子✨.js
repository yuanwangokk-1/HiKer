const csdown = {
              ...
              longClick: [
                {
                  ...
                  js: $.toString(() => {
                    ...
                    let shouye = qzDecrypt(
                      request(
                        "http://c001.22s.mom/encrypt/api.php?path=qiezi/shouye"
                      )
                    );
                    let data = qzDecrypt(
                      request(
                        "http://c001.22s.mom/encrypt/api.php?path=qiezi/zonghe"
                      )
                    );
                    let search = fetch(
                      "http://c001.22s.mom/searchconfig/vipapi/vipconfig.txt"
                    );
                ...
                },
                {
                  ...
                  js: $.toString(() => {
                    var url =
                      "https://api1.yilushunfeng.top|http://rfEXkbyp.yilushunfeng.top|http://rfEXkbyp.changfapiaopiao.top|https://api.changfapiaopiao.top|http://api1.yilushunfeng.top|http://api.changfapiaopiao.top|http://api1.apijiekou.top/api|http://api.phpjiekou.top|http://api.22s.lol/api".split(
                        "|"
                      );
                    var option =
                      "线路1&线路2&线路3&线路4&线路5&线路6&线路7&线路8&线路9".split(
                        "&"
                      );
                    var Line = {
                      title: "切换线路",
  ...
  video: () => {
    ...
    try {
      ...
      //log(getItem('gonggao'))
      //log(getItem('avbk'))
      var list = getItem("shouye")
        .split("首页数据开始")[1]
        .split("首页数据结束")[0]
        .replace(
          /https?\:\/\/(api1?\.)?(changfapiaopiao|yilushunfeng|phpjiekou|apijiekou)\.top(\/api)?/g,
          getItem("host")
        )
        .split("换行");
      list.forEach((data) => {
      ...
      if (getMyVar("a") == "") {
        ...
        const shouye = qzDecrypt(
          request("http://c001.22s.mom/encrypt/api.php?path=qiezi/shouye")
        );
        const data = qzDecrypt(
          request("http://c001.22s.mom/encrypt/api.php?path=qiezi/zonghe")
        );
        const search = fetch(
          "http://c001.22s.mom/searchconfig/vipapi/vipconfig.txt"
        );
...
*/
  ...
  search: () => {
      ...
      if (MY_PAGE == 1) {
        ...
        data.forEach((data) => {
          ...
          if ((data.title != "福利") & !data.platform.includes("yuepao")) {
            d.push({
              title:
                getMyVar("搜索分类", url_n) == url
                  ? strong(data.title, "FF6699")
                  : data.title,
              url: $("#noLoading#").lazyRule(
                (title, url, cate) => {
                  putMyVar("搜索分类", url);
        ...
        var list = JSON.parse(
          fetch(
            getMyVar("搜索分类", "http://003.22s.lol/searchconfig/video.txt")
          )
            .replace("yindangmao", "caoliusp_xilie/yindangmao")
            .replace("erji/missav/missav", "video1/caoliusp_xilie/zhongkoushe")
            .replace("日本专区", "重口社")
        ).searchapi;
        ...
        list.forEach((data) => {
          ...
          d.push({
            title:
              getMyVar(getMyVar("搜索分类名", "视频") + "搜索列表", url_n) ==
              url
                ? strong(data.title, "FF6699")
                : data.title,
            url: $("#noLoading#").lazyRule(
    ...
    try {
      ...
      if (getMyVar("搜索分类名", "视频") == "视频") {
        ...
        getMyVar("搜索分类").includes("actress") |
        (getMyVar("搜索分类名", "视频") == "番号") |
        (getMyVar("搜索分类名", "视频") == "三级")
      ) {
        var Arr = ["shuiguopai"];
        var platform = host
          .match(/top\/.*\.php/)[0]
        ...
        list.forEach((data) => {
          d.push({
            ...
            url:
              wz == "tuzac"
                ? host + "?id=" + data.id + tuzpics
                : host + "?id=" + data.id + pics,
            col_type: "movie_3",
            extra: {
              host: `${host}?id=${data.id}`,
              wz: wz,
  ...
  aes: $.toString(() => {
    ...
    var pics = $("").lazyRule(() => {
      try {
        let img = JSON.parse(fetch(input))
          .content.replace(/\$/g, "")
          .replace(/分割/g, "")
          .split("@");
    ...
    var tvvod = $("").lazyRule(() => {
      ...
      var wsSecret = md5(
        "E9MpC7D5AqWvjmXL3hGkQ2XjZNohAQ" +
          input.replace(/http(s)?:\/\/.*?\//g, "/") +
          t0
      );
    ...
    var hjlazy = $("").image(() => {
      var n = function (e) {
        for (o = "", i = 0, a = 0, void 0; i < e.length; ) {
          ...
          (t = e.charCodeAt(i)) < 128
            ...
            : 191 < t && t < 224
            ? ((a = e.charCodeAt(i + 1)),
              (o += String.fromCharCode(((31 & t) << 6) | (63 & a))),
              (i += 2))
            : ((a = e.charCodeAt(i + 1)),
              (n = e.charCodeAt(i + 2)),
      ...
      var decryptData = function (t) {
        ...
        for (t = t.replace(/[^A-Za-z0-9\*\#]/g, ""); d < t.length; )
          (a = tt.indexOf(t.charAt(d++))),
            (o =
              ((15 & (r = tt.indexOf(t.charAt(d++)))) << 4) |
              ((s = tt.indexOf(t.charAt(d++))) >> 2)),
            (i = ((3 & s) << 6) | (c = tt.indexOf(t.charAt(d++)))),
            (u += String.fromCharCode((a << 2) | (r >> 4))),
            64 != s && (u += String.fromCharCode(o)),
            64 != c && (u += String.fromCharCode(i));
        return n(u);
      };
    ...
    try {
      ...
      if (MY_PAGE == 1) {
        ...
        data.forEach((data) => {
          d.push({
            title:
              getMyVar("首页视频分类" + wz, index_n) == data.id.toString()
                ? strong(data.title, "FF6699")
                : data.title,
            url: $("#noLoading#").lazyRule(
              (title, id, wz) => {
      ...
      var list = JSON.parse(
        fetch(
          host +
            "?sort=" +
            getMyVar("首页视频分类" + wz, getMyVar("index_" + wz)) +
            "&page=" +
            pg
        )
      ).videos;
        ...
        if (Arr.includes(wz)) {
          ...
          if (data.image) {
            ...
            d.push({
              ...
              img:
                img.includes("upload") || img.includes("new.")
                  ? data.image + lazy
                  : data.image,
              url: $("").lazyRule(
                (host, id) => {
  ...
  maomiFM: () => {
    ...
    var 猫咪FM = [
      {
        title:
          "都市激情&家庭乱 伦&另类小说&青春校园&情 色笑话&人 妻乱 伦&性 爱技巧&玄幻奇幻",
    ...
    try {
      ...
      if (MY_PAGE == 1) {
        ...
        keys.forEach((key) => {
          d.push({
            title:
              getMyVar("视频分类" + wz, getMyVar("key_" + wz)) == key
                ? strong(key, "FF6699")
                : key,
            url: $("#noLoading#").lazyRule(
              (key, wz) => {
        ...
        var id_n = getMyVar(
          "视频分类二级" +
            getMyVar("视频分类" + wz, getMyVar("key_" + wz)) +
            wz,
          data[getMyVar("视频分类" + wz, getMyVar("key_" + wz))][0].id + ""
        );
        putMyVar("id_" + wz, id_n);
        data[getMyVar("视频分类" + wz, getMyVar("key_" + wz))].forEach(
          (data) => {
            d.push({
              title:
                getMyVar(
      ...
      if (Arr.includes(wz)) {
        ...
        list.forEach((data) => {
          d.push({
            title: data.title,
            desc:
              parseInt(data.duration / 60) + ":" + parseInt(data.duration % 60),
            img: data.image + lazy,
            url: url + "?id=" + data.id + vod,
            col_type: "movie_2",
          });
        ...
        list.forEach((data) => {
          d.push({
            title: data.title,
            desc:
              (data.date.includes("-") ? data.date : data.created_date) +
              "\t\t\t\t" +
              (data.duration == null ? "" : data.duration),
            img: data.image + lazy,
            url: url + "?id=" + data.id + vod,
    ...
    try {
      ...
      if (MY_PAGE == 1) {
        ...
        data.forEach((data) => {
          d.push({
            title:
              getMyVar("黑料分类" + wz, index_n) == data.id.toString()
                ? strong(data.title, "FF6699")
                : data.title,
            url: $("#noLoading#").lazyRule(
              (title, id, wz) => {
      ...
      var list = JSON.parse(
        fetch(
          host +
            "?sort=" +
            getMyVar("黑料分类" + wz, getMyVar("index_" + wz)) +
            "&page=" +
            pg
        )
      ).videos;
  ...
  blackerji1: () => {
    ...
    try {
      ...
      d.push({
        title:
          pdfh(data.titleCode, "h1&&Text") == ""
            ? pdfh(data.titleCode, "h2&&Text")
            : pdfh(data.titleCode, "h1&&Text"),
        col_type: "rich_text",
      });
      if (wz == "cgw") {
        ...
        texts.forEach((item, index) => {
          ...
          var imgs = pdfa(item, "p&&img").map((h) =>
            pdfh(h, "img&&data-xkrkllgl")
          );
          imgs.forEach((item2) => {
            d.push({
              title: "",
        ...
        try {
          var videos = data.video.replace(/\$/g, "").split("@");
          var img = data.picCode
            .match(/xkrkllgl\=\".*?\".*?alt/)[0]
            .split('"')[1];
          videos.forEach((item, index) => {
            var n = index + 1;
            if (item) {
    ...
    try {
      ...
      if (MY_PAGE == 1) {
        ...
        data.forEach((data) => {
          d.push({
            title:
              getMyVar("图片分类" + wz, index_n) == data.id.toString()
                ? strong(data.title, "FF6699")
                : data.title,
            url: $("#noLoading#").lazyRule(
              (title, id, wz) => {
      ...
      var list = JSON.parse(
        fetch(
          host +
            "?sort=" +
            getMyVar("图片分类" + wz, getMyVar("index_" + wz)) +
            "&page=" +
            pg
        )
      ).videos;
    ...
    try {
      ...
      if (MY_PAGE == 1) {
        ...
        data.forEach((data) => {
          d.push({
            title:
              getMyVar("小说分类" + wz, index_n) == data.id.toString()
                ? strong(data.title, "FF6699")
                : data.title,
            url: $("").lazyRule(
              (title, id, wz) => {
      ...
      var list = JSON.parse(
        fetch(
          host +
            "?sort=" +
            getMyVar("小说分类" + wz, getMyVar("index_" + wz)) +
            "&page=" +
            pg
        )
      ).list;
    ...
    try {
      ...
      if (MY_PAGE == 1) {
        ...
        data.forEach((data) => {
          d.push({
            title:
              getMyVar("帖子分类" + wz, index_n) == data.id.toString()
                ? strong(data.title, "FF6699")
                : data.title,
            url: $("#noLoading#").lazyRule(
              (title, id, wz) => {
      ...
      var list = JSON.parse(
        fetch(
          host + "?sort=" + getMyVar("帖子分类" + wz, index_n) + "&page=" + pg
        )
      ).videos;
      list.forEach((data) => {
        d.push({
    ...
    try {
      ...
      if (MY_PAGE == 1) {
        ...
        data.forEach((data) => {
          d.push({
            title:
              getMyVar("漫画分类" + wz, index_n) == data.id.toString()
                ? strong(data.title, "FF6699")
                : data.title,
            url: $("#noLoading#").lazyRule(
              (title, id, wz) => {
      ...
      var list = JSON.parse(
        fetch(
          host +
            "?sort=" +
            getMyVar("漫画分类" + wz, getMyVar("index_" + wz)) +
            "&page=" +
            pg
        )
      ).videos;
  ...
  manhuaerji1: () => {
    ...
    try {
      ...
      d.push({
        title:
          getVar("shsort") == "1"
            ? '““””<b><span style="color: #FF0000">当前排序：逆序</span></b>'
            : '““””<b><span style="color: #1aad19">当前排序：正序</span></b>',
      ...
      d.push({
        title: (
          "““””预览漫画" +
          (preview == "0" ? "[关]".fontcolor("red") : "[开]".fontcolor("green"))
        ).small(),
        col_type: "text_center_1",
        url: $("#noLoading#").lazyRule(() => {
    ...
    if (分类 == 1) {
      var sign = md5(
        "list_row=20&page=" +
          ...
          t +
          "&type=2" +
          "&m}q%ea6:LDcmS?aK)CeF287bPvd99@E,9Up^"
      );
      var data0 = {
        encode_sign: sign,
        list_row: "20",
      ...
      var sign = md5(
        "timestamp=" + t + "&m}q%ea6:LDcmS?aK)CeF287bPvd99@E,9Up^"
      );
      var data0 = {
        encode_sign: sign,
        timestamp: t,
    ...
    try {
      ...
      var sign = md5(
        "channel=" +
          ...
          "&timestamp=" +
          t +
          "&type=2&m}q%ea6:LDcmS?aK)CeF287bPvd99@E,9Up^"
      );
      let data0 = {
        channel: id,
        encode_sign: sign,
  ...
  baike: () => {
    ...
    try {
      ...
      var pg = getParam("page");
      var data = fetch(
        "https://api1.ffudingdang.com:9002/leixing.php?t=" + mz
      ).split("-");
      var index_n = data[0];
      if (MY_PAGE == 1) {
        data.forEach((data) => {
      ...
      var list = JSON.parse(
        fetch(host + "?t=" + getMyVar("AV百科" + mz, index_n) + "&p=" + pg + sj)
      ).data;
      //log(list)
      list.forEach((data) => {
    ...
    try {
      if (MY_PAGE == 1) {
        ...
        avbkss.forEach((data) => {
          d.push({
            title:
              getMyVar("AV百科搜索", avbkss_n) == data.dz
                ? strong(data.mz, "FF6699")
                : data.mz,
            url: $("#noLoading#").lazyRule(
              (title, url, sj, cate) => {
                putMyVar("AV百科搜索", url);
  ...
  maomi: () => {
    ...
    var 猫咪 = [
      {
        title:
          "猫咪原创&不雅视频&亚洲无 码&抖音妹集合&热剧成人版&韩国三级&人气女 优&国产专区&中文字幕&欧美精品&成人动漫&精品推荐",
    ...
    var sign = Encrypt(
      '{"special_id":' + getMyVar("猫咪", "10") + ',"page":' + pg + "}"
    );
    try {
  ...
  maomimanhua: () => {
    ...
    try {
      ...
      var host =
        "http://43.154.96.251:8089/api/comic/" +
        getMyVar("猫咪漫画", "recommend") +
        "?params=" +
        sign;
      var body = "params=" + sign;
      var html = fetch(host, {
  ...
  maomimeitu: () => {
    ...
    try {
      var 猫咪美图 = [
        {
          title:
            "热门&闲聊讨论&蕾丝女同&美腿玉足&巨乳美胸&男同&原创自拍&动漫&认证&sm&课堂&我妻&高潮脸&网红大咖&制服诱惑&蜜汁美臀",
      ...
      var sign = Encrypt(
        '{"page":' + pg + ',"cate":' + getMyVar("猫咪美图", "0") + ',"type":1}'
      );
  ...
  maomiav: () => {
    ...
    if (getItem("maomi_tp", "") == "") {
      ...
      var config_data = JSON.parse(
        mmDecrypt(config_post.data, config_post.suffix)
      );
    ...
    if (getItem("maomi_categories", "") == "") {
      ...
      var categories_data = mmDecrypt(
        categories_post.data,
        categories_post.suffix
      );
    ...
    if (MY_PAGE == 1) {
      ...
      var cate_library = JSON.parse(getItem("maomi_categories"))
        .apps_categories[getMyVar("maomiav", "library")];
      var cate_library_n = cate_library[0].id + "";
      cate_library.forEach((data, index) => {
        d.push({
          title:
            getMyVar("maomi_cate_library", cate_library_n) == data.id
              ? strong(data.name, "FF6699")
              : data.name,
          url: $("#noLoading#").lazyRule(
            (n, title, id, index) => {
              putMyVar(n, id);
    ...
    var url = `https://mjson.szaction.cc/data/apps/videos/index-${getMyVar(
      "maomi_cate_library",
      cate_library_n
    )}.js`;
    try {
      ...
      var list = JSON.parse(
        mmDecrypt(url_post.data, url_post.suffix)
      ).apps_categories;
      list.forEach((data) => {
        d.push({
        ...
        videos.forEach((data) => {
          d.push({
            title: data.title,
            desc:
              timestampToTime(data.update_time, 1) +
              "   " +
  ...
  maomiav_erji: () => {
    ...
    try {
      var url =
        "https://mjson.szaction.cc/data/apps/videos/list-" +
        id +
        "-" +
        getMyVar("maomisp_cate", "all") +
        "-" +
        getMyVar("maomisp_erji", "latest") +
        "-" +
        pg +
        ".js";
      var url_post = JSON.parse(request(url));
      var list = JSON.parse(mmDecrypt(url_post.data, url_post.suffix)).videos
        .data;
      list.forEach((data) => {
        d.push({
          title: data.title,
    ...
    try {
      ...
      list.forEach((data) => {
        ...
        d.push({
          title: data.title,
          desc: str1,
          pic_url: data.panorama
            .replace("_sync.webp", ".jpg")
            .replace("_longPreview", ""),
          col_type: "pic_1",
          url: data.video_url,
        });
      });
