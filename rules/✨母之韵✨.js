    ...
    try {
      ...
      if (MY_PAGE == 1) {
        ...
        storage0.getItem("cate_").forEach((data) => {
          d.push({
            title:
              getMyVar("cate", getMyVar("index_cate")) == data.id + ""
        ...
        let video_cate_body =
          '{"timestamp":' +
          t +
          ',"version":"0.1.0","os":"h5","nid":' +
          getMyVar("cate", getMyVar("index_cate")) +
          "}";
      ...
      list.forEach((data) => {
        d.push({
          title: data.title,
          desc:
            timestampToTime(data.created_at, 1) +
            "\t\t\t" +
            parseInt(data.video_length / 60) +
            ":" +
            parseInt(data.video_length % 60),
          img: data.horizontal_cover + image,
          url:
            getItem("host") +
            "/api/v2/video/player.m3u8?vid=" +
            data.id +
            "&uid=" +
            home.vipUid,
          col_type: "movie_2",
    ...
    try {
      ...
      list.forEach((data) => {
        d.push({
          title: data.title,
          desc:
            timestampToTime(data.created_at, 0) +
            "\t\t\t" +
            parseInt(data.video_length / 60) +
            ":" +
            parseInt(data.video_length % 60),
